...
export class QueryProvider {
  public static findFirst<T>(datas: T[], query: DynamicQuery<T>): T | undefined {
    const list = this.query(datas, query);
  ...
  public static query<T>(datas: T[], query: DynamicQuery<T>): T[] {
    ...
    if (ArrayUtils.isNotEmpty(query.getSelectedProperties())) {
      cloneDatas = cloneDatas.map((x) => this.pick(query.type, x, query.getSelectedProperties()));
    }

    return cloneDatas;
  }

  private static pick<T>(type: new () => T, data: T, selectedProperties: string[]): T {
